...
export default function AddVehiclePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In a full implementation, we'd fire off /documents/ creation here using the documents state.
      // For now, we simulate saving them.
      
      toast.success('Vehicle and all records saved successfully!');
      router.push('/vehicles');
    } catch (err: unknown) {
      const error = err as any;
      toast.error(error.response?.data?.registration_number?.[0] || 'Failed to save vehicle');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add Truck Master</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Create a comprehensive vehicle profile</p>
          </div>
        </div>
              ...
              {activeTab === 'registration' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Initial Odometer (km)</label>
